<div class="post p-4 bg-white cursor-pointer border-b border-gray-300 w-full max-w-full" onclick="window.location.href='{{ route('posts.show', $post->postid) }}'">
    <div class="post-header mb-1 flex justify-between items-center">
        <div>
            <a href="{{ $post->user->state === 'deleted' ? '#' : route('profile', $post->user->username) }}">
        ...
        @auth
            @if(auth()->id() === $post->userid || Auth::user()->isadmin) 
                <div id= "postOptions-{{ $post->postid }}" class="flex items-center gap-2">
                    <button type="button" onclick="event.stopPropagation(); toggleEditPost('{{ $post->postid }}');" class="text-gray-500 hover:text-black">
    ...
    <!-- Post Topics -->
    <div id = "postTopics-{{ $post->postid }}" class = "flex flex-row gap-2">
    ...
    <div class="post-interactions flex items-center gap-4 mt-4">
        ...
        <div class="post-likes flex items-center gap-2">
            <button type="button" class="flex items-center text-gray-500 hover:text-red-600 group" 
                    onclick="likePost('{{ $post->postid }}', event)">
                <!-- No like -->
                <svg id="heart-empty-{{ $post->postid }}" viewBox="0 0 24 24" aria-hidden="true" 
                    class="h-5 w-5 fill-gray-500 hover:fill-red-600 group-hover:fill-red-600 {{ $post->liked ? 'hidden' : '' }}">
                    <g>
                ...
                <!-- Yes like -->
                <svg id="heart-filled-{{ $post->postid }}" viewBox="0 0 24 24" aria-hidden="true" 
                    class="h-5 w-5 fill-red-600 group-hover:fill-red-600 {{ $post->liked ? '' : 'hidden' }}">
                    <g>
        ...
        <!-- Comment Button -->
        <div class="post-posts flex items-center gap-2">
            <button 
                type="button" 
                ...
                <!-- Comment Count -->
                <span id="post-count-{{ $post->postid }}" 
            ...
            <div id="edit-post-{{ $post->postid }}" class="edit-post-form hidden mt-4 bg-white rounded-xl shadow-md p-4" onclick="event.stopPropagation();">
                <form action="{{ route('posts.update', $post->postid) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4" data-post-id="{{ $post->postid }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="hidden" id="topicInput-{{ $post->postid }}" class="topicInput" name="topics[]" value="[]" >
